...
In this section, we introduce a new functional form for thermal performance curves called flexTPC. This model aims to be both flexible (aiming to describe unimodal TPC curves of any skewness) and interpretable (with all model parameters having a clear biological interpretation).
...
Every functional form in bayesTPC needs to be written down as an expression in R. Let's start by writing down an expression for the flexTPC equation.
...
When using MCMC methods, we obtain samples from the posterior distribution of the parameters in our model. For example, let's take a look at the first few samples from the condition with no antibiotics.
...
(Optional exercise: Try calculating the posterior distribution for the optimal temperature of the TPC for a condition where antibiotics are present.)
...
# FlexTPC equation. This is equivalent to the equation shown in the visualization above, although it's written a little differently.
...
Now we have posterior samples for the TPC (y-axis) value at each of the (x-axis) temperature values in our grid. We can now plot the medians and 95% credible intervals for each of these values.

(Note: Please make sure you have the `scales` package, as we'll need it in order to make partially transparent shaded regions for credible intervals).
...
Note that we could use the same functional form (the flexTPC equation) for all of these conditions despite their different shapes. Using a flexible model allows us to directly compare the inferred parameters thermal performance curves that vary in shape like in this example.
