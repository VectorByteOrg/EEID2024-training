...
```
Each curve we're plotting corresponds to the flexTPC model when the parameters are fixed at the current iteration of the MCMC chain. With Bayesian methods, rather than fitting a single curve we have a posterior distribution of possible curves that are consistent with the data and priors. We can then summarize this posterior distribution with e.g. the median and 95% credible interval y-axis value at each temperature in our grid.

Now we can plot the medians and 95% credible intervals for each of these temperature values for all antibiotic conditions.
